"""
Minimal Vercel serverless function for handling Telegram webhook.
Uses only Python standard library and urllib for HTTP requests.
"""
...
app = FastAPI(title="Telegram Webhook Minimal")
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        if message_info:
            ...
            send_simple_response(message_info)
...
async def webhook_info():
    ...
    return JSONResponse({
        "status": "ok",
        "message": "Telegram webhook endpoint (minimal version) is operational",
...
def send_simple_response(message_info):
    """Send a simple response back to the user using urllib."""
    try:
        ...
        if user_content.lower().startswith('/start'):
            ...
            response_text = f"📩 Hi {first_name}! I received your message: \\"{user_content}\\"\\n\\n🤖 I'm an AI assistant that helps with document processing and analysis. I'm currently in minimal mode while we work on the full features!"
